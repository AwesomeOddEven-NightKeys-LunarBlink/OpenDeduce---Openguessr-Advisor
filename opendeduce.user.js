...
// @version      1.0.0
// @description  Geo-Deduction Engine v1.0.0 - Clean Logic with Separate rules.json Database.
...
(function() {
    ...
    const STYLES = `
        ...
        #od-panel { position: fixed; width: 360px; max-height: 90vh; background: rgba(18,18,22, 0.98); backdrop-filter: blur(40px) saturate(180%); border: 1px solid rgba(255,255,255, 0.15); border-radius: 30px; color: #f8fafc; font-family: 'Plus Jakarta Sans', sans-serif; z-index: 10000; box-shadow: 0 40px 120px #000; display: flex; flex-direction: column; }
        ...
        .od-acc-body { padding: 4px 22px 22px; border-top: 1px solid #ffffff05; display: block; }
        ...
        .od-suspect-list { max-height: 220px; overflow-y: auto; scrollbar-width: thin; scrollbar-color: #ffffff22 transparent; }
        ...
        .od-score { font-family: 'JetBrains Mono'; font-weight: 800; color: #10b981; min-width: 50px; text-align: right; }
    `;
    ...
    function updateSuspects() {
        ...
        countText.innerText = `${hiProb} High-Probability suspects`;
        meter.style.width = ((hiProb / STATE.countries.length)*100)+'%';
        container.innerHTML = sorted.map(c => `<div class="od-suspect-row" style="opacity:${Math.max(0.3, c.score)}"><span>${c.name}</span><span class="od-score">${Math.round(c.score*100)}%</span></div>`).join('');
    }
    ...
    async function init() {
        const dbText = GM_getResourceText("DATABASE");
        if(dbText) {
            const db = JSON.parse(dbText);
            STATE.countries = db.countries;
            STATE.rules = db.rules;
        } else {
            console.error("OpenDeduce: Database fetch failed. Please check your GitHub raw link.");
        }
        ...
        p.innerHTML = `
            ...
            <div id="od-hud-body" style="display:${STATE.isMinimized?'none':'block'}">
                <div class="od-search-box"><input type="text" id="od-search" class="od-input" placeholder="Search Database..."><div id="od-suggest" class="od-suggestions"></div></div>
                <div class="od-active-bar"></div><div class="od-content" id="od-content"></div>
                <div class="od-footer"><div class="od-res-meta"><span id="od-count">Loading...</span><span>Likelihood</span></div>
