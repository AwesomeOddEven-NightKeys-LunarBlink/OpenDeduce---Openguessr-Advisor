...
// @version      1.0.0
// @description  Geo-Deduction Engine v1.0.0 - Corrected Elimination Logic and Deep Search.
...
(function() {
    ...
    const STYLES = `
        ...
        #od-v1-panel {
            ...
            background: rgba(18,18,22, 0.95); backdrop-filter: blur(40px) saturate(180%);
            border: 1px solid rgba(255,255,255, 0.15); border-radius: 30px;
            color: #f8fafc; font-family: 'Plus Jakarta Sans', sans-serif;
            z-index: 10000; display: flex; flex-direction: column;
            box-shadow: 0 40px 120px rgba(0,0,0,0.9); transition: max-height 0.3s;
        }
        #od-v1-panel.minimized { max-height: 78px; overflow: hidden; }

        .od-header { padding: 24px 28px; border-bottom: 1px solid #ffffff11; cursor: move; display: flex; justify-content: space-between; align-items: center; }
        ...
        .od-clue-item input { width: 17px; height: 17px; accent-color: #60a5fa; }
        ...
        .od-suspects { max-height: 180px; overflow-y: auto; }
        .od-country-row { display: flex; justify-content: space-between; padding: 8px 6px; font-size: 0.9rem; }
    `;
    ...
    function updateSuspects() {
        ...
        STATE.activeClueIds.forEach(id => {
            ...
            list.forEach(country => {
                ...
                // ELIMINATION LOGIC: If box checked, eliminate those NOT matching.
        ...
        count.innerText = `${sorted.length} Suspects Remaining`;
        meter.style.width = ((sorted.length / STATE.countries.length) * 100) + '%';
        container.innerHTML = sorted.map(c => `<div class="od-country-row" style="opacity:${Math.max(0.4, c.score)}"><span>${c.name}</span><span style="font-family:monospace; color:#10b981;">${Math.round(c.score*100)}%</span></div>`).join('');
    }
    ...
    function setupSearch() {
        ...
        i.oninput = (e) => {
            ...
            STATE.rules.forEach(g => {
                const isThemeMatch = g.category.toLowerCase().includes(v);
                g.clues.forEach(cl => {
                    if (isThemeMatch || cl.aspect.toLowerCase().includes(v) || (cl.description||"").toLowerCase().includes(v)) {
                        matches.push({...cl, category: g.category});
                    }
                });
            });
            if(matches.length > 0) {
    ...
    function renderAccordion(filter="") {
        ...
        STATE.rules.forEach(g => {
            const isThemeMatch = g.category.toLowerCase().includes(filter);
            const clues = g.clues.filter(f => isThemeMatch || f.aspect.toLowerCase().includes(filter) || (f.description||"").toLowerCase().includes(filter));
    ...
    async function init() {
        STATE.countries = [
            {"id":"fr","name":"France","continent":"Europe"},{"id":"ru","name":"Russia","continent":"Asia"},{"id":"us","name":"United States","continent":"North America"},{"id":"jp","name":"Japan","continent":"Asia"},{"id":"au","name":"Australia","continent":"Oceania"},{"id":"uk","name":"United Kingdom","continent":"Europe"},{"id":"ie","name":"Ireland","continent":"Europe"},{"id":"za","name":"South Africa","continent":"Africa"},{"id":"br","name":"Brazil","continent":"South America"},{"id":"cl","name":"Chile","continent":"South America"},{"id":"ng","name":"Nigeria","continent":"Africa"},{"id":"id","name":"Indonesia","continent":"Asia"}
        ];
        ...
        STATE.rules = [
            ...
            { "category": "Theme: Flora & Botany", "clues": [
                { "id":"t1", "aspect":"Jacaranda (Violet Flowers)", "confidence":0.8, "excludeContinents":["Europe"] }
            ]}
        ];
        ...
        p.innerHTML = `
            ...
            <div id="od-hud-body" style="display: ${STATE.isMinimized ? 'none' : 'block'}">
                <div class="od-search-area"><input type="text" id="od-search" class="od-input" placeholder="Search (e.g. 'Right', 'Driving Side')..."><div id="od-suggest" class="od-suggestions"></div></div>
                <div class="od-active-bar"></div><div class="od-content" id="od-content"></div>
                <div class="od-footer"><div class="od-sus-meta"><span id="od-count">Calculating...</span><span>Likelihood</span></div>
